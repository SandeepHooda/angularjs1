//Start of file angular-ui-router.js
...
//End of file angular-ui-router.js

//Start of file highstock.js

/*
 Highstock JS v1.3.7 (2013-10-24)
\\ms\dev\ria\highstocks\1.3.7\install\common\js\highstock.js
...
//End of file highstock.js


//Start of file ngIdle.js
'use strict';
...
function $KeepaliveProvider() {
    ...
    this.$get = function ($rootScope, $log, $timeout) {
        var state = {ping: null};
        ...
        function ping(onetimeonly) {
            if($rootScope.idlemode) {
                $rootScope.$broadcast('$keepalive');
            }
            $rootScope.idlemode = false;
            schedulePing();
        };
...
function $IdleProvider() {
    ...
    var options = {
        ...
        events: 'mousemove keydown DOMMouseScroll mousewheel mousedown touchstart scroll',
        keepalive: true
    };
    ...
    this.$get = function ($timeout, $log, $rootScope, $document, $keepalive) {
        ...
        var interrupt = function () {
            ...
            if($rootScope.eventFire == true || $rootScope.eventFire == undefined) {
                $rootScope.idlemode = true;
...
//End of file ngIdle.js


//Start of file nprogress.js
...
//End of file nprogress.js

//Start of file scroller.js

/**
 * Created by singsume on 4/21/14.
 */

angular.module('ui.scrollfix', []).directive('scroller', ['$window','$timeout', function ($window,$timeout) {
    return {
        link: function postLink(scope,ele,attrs) {
            var $winTarget = angular.element($window);
            var $heightTarget = angular.element("#"+attrs.heightTarget);
            var heightTargetOffset;
            var heightTargetPos;
            var windowOffset = 0;
            var $offsetTopele;
            var chartOffset ;
            ...
            /*Watch for waiting till the data is populated*/
            scope.$watch(attrs.watcher,function()
            {
                if(scope[attrs.watcher]) {
                    /*Timeout for rendering charts if we refreshed the same page*/
                    cancelTimeout = $timeout(bindScroll,1000);
                    // bindScroll();
                }
            });

            /*Register constant values for positions and offsets*/
            function bindScroll() {
                $timeout.cancel(cancelTimeout);
                $heightTarget = angular.element("#"+attrs.heightTarget);
                $winTarget.off('scroll',bindScroll);
                heightTargetOffset = $heightTarget[0].offsetHeight-10;
                heightTargetPos = $heightTarget[0].offsetTop;
                windowOffset = 0;

                ele.css('top','');

                chartOffset = ele[0].offsetHeight;
                chartPos = ele[0].offsetTop;

                if(attrs.offsetTopele) {
                    $offsetTopele = angular.element('#'+attrs.offsetTopele);
                    heightTargetPos = $offsetTopele[0].offsetTop;
                    chartPos = $offsetTopele[0].offsetTop;
                }
                $winTarget.on('scroll', onScroll);
            }
            ...
            function onScroll() {
                ...
                if (angular.isDefined($window.pageYOffset)) {
                    ...
                    windowOffset = iebody.scrollTop;
                }

                if(attrs.dynamicTargetheight) {
                    heightTargetOffset = $heightTarget[0].offsetHeight-10;
                    chartOffset = ele[0].offsetHeight;
                }
                ...
                if(chartOffset > heightTargetOffset) {
                    $winTarget.off('scroll',bindScroll);
                    return;
                }


                var pixelsPassed = windowOffset - chartPos;

                if(pixelsPassed< 0) {
                    pixelsPassed = 0;
                }
                //console.log("pixels passed : "+pixelsPassed);
                if(pixelsPassed > 0 && pixelsPassed < (heightTargetOffset - chartOffset)) {
                    ele.css('top',pixelsPassed+'px');
                }

                if(windowOffset < chartPos) {
                    ele.css('top','0px');
                }
                else if(pixelsPassed >= heightTargetOffset - chartOffset) {
                    ele.css('top',(heightTargetOffset - chartOffset)+'px');
                }
            }

            scope.$on('$destroy', function () {
                $winTarget.off('scroll', onScroll);
            });
        }
    }
